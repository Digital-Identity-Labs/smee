defmodule Smee.Fetch do
  ...
  """
  @spec remote(source :: Source.t(), options :: keyword()) ::
          {:ok, Metadata.t()} | {:error, any()}
    ...
    if Utils.file_url?(source.url) do
      try do
        ...
        case File.read(file_path) do
          {:ok, data} ->
            xml = metadata_from_file(file_path, data, source)
            {:ok, xml}

          {:error, message} ->
            {:error, "Could not open and read file #{source.url} (#{message})"}
        end
      rescue
  ...
  def probe(source) do
    if Utils.file_url?(source.url) do
      ...
      case Req.head(source.url, http_options(source, cache: false)) do
        ...
        {:ok, %{status: status}} ->
          {:error, "Probing #{source} resulted in a #{status} status"}

        {:error, _} ->
          {:error, "Cannot probe #{source}"}
      end
    end
  end
  ...
  defp check_http_data_type!(source, %{status: 200} = response) do
    ...
    if type != "application/samlmetadata+xml" && type != "application/xml" && type != "text/xml" do
      if source.strict do
        ...
        IO.warn(
          "Data from #{source.url} is not described as SAML metadata (application/samlmetadata+xml) - type is actually #{type}",
          []
        )
      end
    end
  ...
  @spec metadata_from_response(url :: binary(), response :: struct, source :: Source.t()) ::
          {:ok, Metadata.t()}
          | {
              :error,
              atom()
      ...
      200 ->
        {
          ...
          Smee.Metadata.new(
            ...
            modified_at:
              Smee.Utils.parse_http_datetime(Req.Response.get_header(response, "last-modified")),
            downloaded_at:
              Smee.Utils.parse_http_datetime(Req.Response.get_header(response, "date")),
  ...
  defp warm_get_task(source) do
    Task.async(fn ->
      case Req.get(
             Utils.fetchable_remote_xml(source),
             Keyword.put(http_options(source), :cache, true)
           ) do
