defmodule Smee.XmlMunger do
  ...
  def expand_entity_top(xml, options \\ []) do
    ...
    vu_fragment =
      if is_nil(valid_until), do: nil, else: ~s|validUntil="#{Utils.valid_until(valid_until)}"|
  ...
  def split_aggregate_to_stream(xml, _options \\ []) do
    ...
    |> Stream.reject(fn xf ->
      String.starts_with?(xf, ["</EntitiesDescriptor>", "</md:EntitiesDescriptor>"])
    end)
  end
  ...
  def snip_aggregate(xml) do
    ...
    case Regex.run(entities_descriptor_pattern(), xml) do
      ...
      nil ->
        raise "Can't extract EntitiesDescriptor! Data was: #{String.slice(xml, 0..1000)}[...]"
    end
  end
