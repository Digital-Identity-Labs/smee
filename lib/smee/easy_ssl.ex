...
defmodule Smee.EasySSL do
  ...
  def parse_der(certificate_der, opts \\ [all_domains: false, serialize: false])
      when is_binary(certificate_der) do
  ...
  def parse_pem(cert_charlist) when is_list(cert_charlist) do
    parse_pem(cert_charlist |> to_string())
  end

  def parse_pem(cert_pem, opts \\ [all_domains: false, return_base64: false]) do
    cert_regex =
      ~r/^\-{5}BEGIN\sCERTIFICATE\-{5}\n(?<certificate>[^\-]+)\-{5}END\sCERTIFICATE\-{5}/
        ...
        |> Enum.reduce(%{}, fn extension, extension_map ->
          case extension do
              ...
              Map.put(
                ...
                |> Enum.reduce([], fn entry, issuer_list ->
                  case entry do
                    ...
                    {:iPAddress, ip} ->
                      ["IP:" <> (ip |> ip_to_string()) | issuer_list]
            ...
            {:Extension, {2, 5, 29, 35}, _critical,
             {:AuthorityKeyIdentifier, authority_key_identifier, _, _}} ->
