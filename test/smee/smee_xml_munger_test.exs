defmodule SmeeXmlMungerTest do
  ...
  describe "xml_declaration/0" do
    test "returns a big XML declaration" do
      assert XmlMunger.xml_declaration() ==
  ...
  describe "remove_xml_declaration/1" do
    ...
    test "removes small declarations" do
      assert "<test>content</test>" =
               XmlMunger.remove_xml_declaration(~s|<?xml version="1.0" ?>\n<test>content</test>|)
    end

    test "removes big declarations" do
      assert "<test>content</test>" =
               XmlMunger.remove_xml_declaration(
                 ~s|<?xml version="1.0" encoding="UTF-8" standalone="no"?>\n<test>content</test>|
               )
    end
    ...
    test "removes XML declarations on the same line" do
      assert "<test>content</test>" =
               XmlMunger.remove_xml_declaration(~s|<?xml version="1.0" ?><test>content</test>|)
    end
  end
  ...
  describe "expand_entity_top/2" do
    ...
    test "entity XML with a thin top returns XML with a fat top" do
      assert "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?>\n<EntityDescriptor xmlns=\"urn:oasis:names:tc:SAML:2.0:metadata\" xmlns:ds=\"http://www.w3.org/2000/09/xmldsig#\" xmlns:md=\"urn:oasis:names:tc:SAML:2.0:metadata\" xmlns:mdrpi=\"urn:oasis:names:tc:SAML:metadata:rpi\" xmlns:mdui=\"urn:oasis:names:tc:SAML:metadata:ui\" xmlns:shibmd=\"urn:mace:shibboleth:metadata:1.0\" xmlns:ui=\"urn:oasis:names:tc:SAML:metadata:ui\" cacheDuration=\"P0Y0M0DT6H0M0.000S\" entityID=\"https://indiid.net/idp/shibboleth\">" <>
               _ =
               XmlMunger.expand_entity_top(@valid_single_metadata_xml, [])
    end
  ...
  describe "extract_uri!/1" do
    test "extracts the entityID from a single entity's XML" do
      assert "https://indiid.net/idp/shibboleth" =
               XmlMunger.extract_uri!(@valid_single_metadata_xml)
    end
    ...
    test "can cope with unusual namespaces" do
      xml =
        String.replace(@valid_single_metadata_xml, "<EntityDescriptor", "<q1:EntityDescriptor")
  ...
  describe "generate_aggregate_header/1" do
    ...
    test "by default includes all known namespaces" do
      ...
      assert ^known_namespaces =
               XmlMunger.generate_aggregate_header([])
    ...
    test "validUntil can be set to a number of days in the future by passing an integer to the option [rather circular test]" do
      ...
      assert String.contains?(
               XmlMunger.generate_aggregate_header(valid_until: 20),
               ~s| validUntil="#{expected_string}|
             )
    end
  ...
  describe "split_aggregate_to_stream/2" do
    test "returns a stream of entity xml fragments" do
      ...
      assert 2 =
               XmlMunger.split_aggregate_to_stream(@valid_metadata_xml)
               |> Enum.to_list()
               |> Enum.count()
    end
  end
  ...
  describe "split_single_to_stream/2" do
    test "returns a stream of entity xml fragments" do
      ...
      assert 1 =
               XmlMunger.split_single_to_stream(@valid_single_metadata_xml)
               |> Enum.to_list()
               |> Enum.count()
    end
  end
  ...
  describe "snip_aggregate/1" do
    test "returns the EntityDescriptors tag from aggregate metadata" do
      ...
      assert String.ends_with?(
               XmlMunger.snip_aggregate(@valid_metadata_xml),
               ~s| cacheDuration=\"PT6H0M0.000S\">|
             )
    end
  end
  ...
  describe "consistent_bottom/2" do
    test "makes sure the entityDescriptor end tag matches the namespace style of the start tag (currently by making both use default ns)" do
      assert "blah blah </EntityDescriptor>" =
               XmlMunger.consistent_bottom("blah blah </md:EntityDescriptor>")

      assert "blah blah </EntityDescriptor>" =
               XmlMunger.consistent_bottom("blah blah </EntityDescriptor>")
    end
  end
  ...
  describe "process_metadata_xml/2" do
    ...
    test "removes small declarations" do
      assert "<test>content</test>" =
               XmlMunger.process_metadata_xml(~s|<?xml version="1.0" ?>\n<test>content</test>|)
    end
    ...
    test "removes XML declarations on the same line" do
      assert "<test>content</test>" =
               XmlMunger.process_metadata_xml(~s|<?xml version="1.0" ?><test>content</test>|)
    end

    test "removes surrounding whitespace, line endings, etc" do
      assert "<test>content</test>" =
               XmlMunger.process_metadata_xml("   <test>content</test>  \n")
    end

    test "removes multiple blank lines" do
      assert "<test>\ncontent\n</test>" =
               XmlMunger.process_metadata_xml("   <test>\n\n\ncontent\n\n\n</test>\n\n\n")
    end

    test "removes groups (embedded EntityDescriptors)" do
      assert 8 =
               Regex.scan(@all_eds_pattern, @complex_metadata_xml)
               |> Enum.count()

      assert 2 =
               Regex.scan(@all_eds_pattern, XmlMunger.process_metadata_xml(@complex_metadata_xml))
               |> Enum.count()
    end

    test "process valid XML should still be valid XML" do
      ## TODO: change to validate when ID issue is fixed
      assert {:ok, _} =
               XmlMunger.process_metadata_xml(@complex_metadata_xml) |> Smee.Lint.well_formed()
    end
  end
  ...
  describe "remove_groups/1" do
    test "remove all but the top opening and closing EntityDescriptor tags" do
      ...
      assert 2 =
               Regex.scan(@all_eds_pattern, XmlMunger.remove_groups(@complex_metadata_xml))
               |> Enum.count()
    end
  end
