defmodule SmeeEntityTest do
  ...
  describe "new/2" do
    ...
    test "entity_id is only set automatically and cannot be set with options" do
      assert %Entity{uri: "https://indiid.net/idp/shibboleth"} =
               Entity.new(@valid_xml, uri: "https://example.org/idp")
    end

    test "sets data hash automatically" do
      assert %Entity{data_hash: "d5bae57b05967ef45dc00c301c80276f9e4b4fa3"} =
               Entity.new(@valid_xml)
    end
    ...
    test "metadata_uri_hash is set automatically if metadata uri is present" do
      %Entity{metadata_uri_hash: "797e00e36df8100d422bc6901b21ebf7f8bc58e1"} =
        Entity.new(
          @valid_xml,
    ...
    test "a parsed xmlerl structure is included automatically by default" do
      ...
      assert %{uri: "https://indiid.net/idp/shibboleth"} =
               xdoc
  ...
  describe "derive/2" do
    ...
    test "sets data hash automatically" do
      assert %Entity{data_hash: "d5bae57b05967ef45dc00c301c80276f9e4b4fa3"} =
               Entity.derive(@valid_xml, @valid_metadata)
    end

    test "metadata_uri defaults to the name of the metadata" do
      assert %Entity{metadata_uri: "http://example.com/federation"} =
               Entity.derive(@valid_xml, @valid_metadata)
    end

    test "metadata_uri_hash defaults to the hash of the name of the metadata" do
      assert %Entity{metadata_uri_hash: "797e00e36df8100d422bc6901b21ebf7f8bc58e1"} =
               Entity.derive(
                 @valid_xml,
                 @valid_metadata
               )
    ...
    test "downloaded_at can be overridden with options" do
      %Entity{downloaded_at: @arbitrary_dt} =
        Entity.derive(@valid_xml, @valid_metadata, downloaded_at: @arbitrary_dt)
    end

    test "modified_at can be overridden with options" do
      %Entity{modified_at: @arbitrary_dt} =
        Entity.derive(@valid_xml, @valid_metadata, modified_at: @arbitrary_dt)
    end

    test "valid_until can be overridden with options" do
      %Entity{valid_until: @arbitrary_dt} =
        Entity.derive(@valid_xml, @valid_metadata, valid_until: @arbitrary_dt)
    end
    ...
    test "metadata_uri can be overridden with options" do
      %Entity{metadata_uri: "http://example.com/federation"} =
        Entity.derive(
          @valid_xml,
          @valid_metadata,
          metadata_uri: "http://example.com/federation"
    ...
    test "trustiness can be set with options" do
      assert %Entity{trustiness: 0.2} =
               Entity.derive(@valid_xml, @valid_metadata, trustiness: 0.2)
    end
  ...
  describe "update/2" do
    ...
    test "an updated entity is decompressed" do
      assert %Entity{compressed: false} =
               Entity.update(Entity.compress(@valid_entity), @updated_xml)
    end
    ...
    test "an updated entity has the correct data hash" do
      assert %Entity{data_hash: "c84993812fdc137f1f94410aeecff93b9463773d"} =
               Entity.update(@valid_entity, @updated_xml)
    end
  ...
  describe "filename/1" do
    ...
    test "return a suggested filename for the entity in sha1 format" do
      assert "77603e0cbda1e00d50373ca8ca20a375f5d1f171.xml" =
               Entity.filename(@valid_entity, :sha1)
    end
  ...
  describe "trustiness/1" do
    ...
    test "cannot return a trustiness under 0" do
      assert "0.0" =
               Entity.trustiness(struct(@valid_entity, %{trustiness: -0.5}))
               |> Float.to_string()
    end
  end
  ...
  describe "categories/1" do
    test "returns a list of entity categories" do
      assert ["http://refeds.org/category/research-and-scholarship"] =
               Entity.categories(@proxy_entity)
    end
  ...
  describe "category_support/1" do
    test "returns a list of entity supported entity categories" do
      assert ["http://refeds.org/category/research-and-scholarship"] =
               Entity.category_support(@proxy_entity)
    end
