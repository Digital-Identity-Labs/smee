defmodule SmeeMetadataTest do
  ...
  describe "new/2" do
    ...
    test "uri is set to the URI name of aggregate metadata, if present, entityID of single, or nil if absent" do
      ...
      assert %Metadata{uri: "https://indiid.net/idp/shibboleth"} =
               Metadata.new(
    ...
    test "uri_hash is set to the sha1 hash of the URI name of the metadata, if present, or nil" do
      assert %Metadata{uri_hash: "797e00e36df8100d422bc6901b21ebf7f8bc58e1"} =
               Metadata.new(@valid_metadata_xml)

      assert %Metadata{uri_hash: "77603e0cbda1e00d50373ca8ca20a375f5d1f171"} =
               Metadata.new(
                 @valid_single_metadata_xml,
                 type: :single
               )
    ...
    test "url_hash is set to sha1 hash of url, if it's set" do
      assert %Metadata{url_hash: "c28e9841771a33bf7bc9ca45769288de3da9b8b3"} =
               Metadata.new(
                 @valid_metadata_xml,
    ...
    test "data_hash is set automatically to the sha1 hash of the data" do
      assert %Metadata{data_hash: "7bb9e69f7b5490f679e70b9fc4e4b14d2022ab83"} =
               Metadata.new(@valid_metadata_xml)
    end
    ...
    test "valid_until defaults to the validity in the XML data, or nil" do
      ...
      assert %Metadata{valid_until: ~U[2021-12-25 17:33:22.438Z]} =
               Metadata.new(
                 @valid_single_metadata_xml,
                 type: :single
               )
    ...
    test "etag defaults to the data hash" do
      assert %Metadata{etag: "7bb9e69f7b5490f679e70b9fc4e4b14d2022ab83"} =
               Metadata.new(@valid_metadata_xml)
    end
    ...
    test "downloaded_at can be set using an option" do
      ...
      assert %Metadata{downloaded_at: ^now} =
               Metadata.new(@valid_metadata_xml, downloaded_at: now)
    end
    ...
    test "url can be set using an option" do
      assert %Metadata{url: "http://example.com/metadata.xml"} =
               Metadata.new(
                 @valid_metadata_xml,
                 url: "http://example.com/metadata.xml"
               )
    ...
    test "etag can be set using an option" do
      assert %Metadata{etag: "9EC1E756-5752-4B8E-8261-32B9DD0F2D40"} =
               Metadata.new(
                 @valid_metadata_xml,
    ...
    test "label can be set using an option" do
      assert %Metadata{label: "Testing Here"} =
               Metadata.new(@valid_metadata_xml, label: "Testing Here")
    end

    test "uri can be set using an option" do
      assert %Metadata{uri: "http://example.com/federation"} =
               Metadata.new(
                 @valid_metadata_xml,
    ...
    test "cert_url can be set using an option" do
      assert %Metadata{cert_url: "http://metadata.ukfederation.org.uk/ukfederation.pem"} =
               Metadata.new(
                 @valid_metadata_xml,
    ...
    test "cert_fingerprint can be set using an option" do
      assert %Metadata{cert_fingerprint: "0baa09b8fedaa809bd0e63317afcf3b7a9aedd73"} =
               Metadata.new(
                 @valid_metadata_xml,
    ...
    test "data cannot be set using an option" do
      ...
      assert %Metadata{data: ^data} =
               Metadata.new(@valid_metadata_xml, data: "This shouldn't be set")
    end
  ...
  describe "derive/2" do
    test "returns an :aggregate type Metadata struct when passed a stream of entities" do
      assert %Metadata{type: :aggregate} =
               Metadata.derive(Metadata.stream_entities(@valid_metadata))
    end
    ...
    test "size is set automatically to the bytesize of the data" do
      assert %Metadata{size: 41_381} = Metadata.derive(Metadata.entities(@valid_metadata))
    end
    ...
    test "cert_fingerprint defaults to nil" do
      assert %Metadata{cert_fingerprint: nil} =
               Metadata.derive(Metadata.entities(@valid_metadata))
    end
    ...
    test "downloaded_at can be set using an option" do
      ...
      assert %Metadata{downloaded_at: ^now} =
               Metadata.derive(Metadata.entities(@valid_metadata), downloaded_at: now)
    end
    ...
    test "modified_at can be set using an option" do
      ...
      assert %Metadata{modified_at: ^now} =
               Metadata.derive(Metadata.entities(@valid_metadata), modified_at: now)
    end
    ...
    test "priority can be set using an option" do
      assert %Metadata{priority: 8} =
               Metadata.derive(Metadata.entities(@valid_metadata), priority: 8)
    end

    test "trustiness can be set using an option" do
      assert %Metadata{trustiness: 0.8} =
               Metadata.derive(Metadata.entities(@valid_metadata), trustiness: 0.8)
    end

    test "data cannot be set using an option" do
      %Metadata{data: data} =
        Metadata.derive(Metadata.entities(@valid_metadata), data: "This shouldn't be set")
    ...
    test "count cannot be set using an option" do
      assert %Metadata{entity_count: 2} =
               Metadata.derive(Metadata.entities(@valid_metadata), entity_count: 20)
    end

    test "size cannot be set using an option" do
      assert %Metadata{size: 41_381} =
               Metadata.derive(Metadata.entities(@valid_metadata), size: 100)
    end

    test "verified cannot be set using an option" do
      assert %Metadata{verified: false} =
               Metadata.derive(Metadata.entities(@valid_metadata), verified: true)
    end
  ...
  describe "update/2" do
    ...
    test "updated metadata is decompressed" do
      assert %Metadata{compressed: false} =
               Metadata.update(Metadata.compress(@valid_metadata), @updated_xml)
    end
    ...
    test "updated metadata has the correct data hash" do
      assert %Metadata{data_hash: "5aa7f95301cc69fe7161d954abd8ca551608791e"} =
               Metadata.update(
                 @valid_metadata,
                 @updated_xml
               )
  ...
  describe "xdoc/1" do
    ...
    test "returns parsed xmerl data if XML binary string is present in the struct" do
      ...
      assert %{uri: "http://example.com/federation"} =
               xdoc
  ...
  describe "entity/2" do
    test "returns entity record for the specified entityID if present in metadata" do
      assert %Entity{uri: "https://test.ukfederation.org.uk/entity"} =
               Metadata.entity(
                 @valid_metadata,
  ...
  describe "filename/2" do
    ...
    test "return a suggested filename for the Metadata in sha1 format" do
      assert "797e00e36df8100d422bc6901b21ebf7f8bc58e1.xml" =
               Metadata.filename(@valid_metadata, :sha1)
    end
  ...
  describe "validate!/1" do
    ...
    test "raises an exception if metadata XML is invalid" do
      assert_raise(
        RuntimeError,
        fn ->
          Metadata.validate!(struct(@valid_metadata, %{data: @valid_metadata.data <> "BAD"}))
        end
      )
    end
  end
