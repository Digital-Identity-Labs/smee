defmodule SmeeFilterTest do
  ...
  describe "tag/3" do
    test "only entities with a matching tag remain" do
      ...
      assert [%Entity{uri: "https://test.ukfederation.org.uk/entity"}] =
               Stream.concat([[tagged_entity, @idp_entity, @proxy_entity]])
               |> Filter.tag("walrus")
               |> Enum.to_list()
    end
  ...
  describe "entity_category/3" do
    test "only entities with a matching entity category remain" do
      assert [%Entity{uri: "https://cern.ch/login"}] =
               Stream.concat([[@sp_entity, @idp_entity, @proxy_entity]])
               |> Filter.entity_category("http://refeds.org/category/research-and-scholarship")
               |> Enum.to_list()
    end
  ...
  describe "registered_by/3" do
    test "only entities with a matching registrar URI remain" do
      assert [
               ...
               Stream.concat([[@sp_entity, @local_adfs_entity, @proxy_entity]])
               |> Filter.registered_by("http://ukfederation.org.uk")
               |> Enum.to_list()
    end
  ...
  describe "registered_before/3" do
    test "only entities with a matching registration time will remain" do
      assert [
               ...
               Stream.concat([[@sp_entity, @idp_entity, @local_adfs_entity, @proxy_entity]])
  ...
  describe "fresh/2" do
    test "only entities registered in the last week will remain" do
      ...
      assert [
               ...
               Stream.concat([[tweaked_sp, @idp_entity, @local_adfs_entity, @proxy_entity]])
               |> Filter.fresh()
               |> Enum.to_list()
    end
