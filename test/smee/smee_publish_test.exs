defmodule SmeePublishTest do
  ...
  describe "write_items/2" do
    ...
    test "returns a list of filenames after writing all items to disk", %{tmp_dir: tmp_dir} do
      ...
      assert Enum.all?(
               Publish.write_items(md_stream, format: :disco, to: tmp_dir),
               &File.exists?/1
             )

      assert Enum.all?(
               Publish.write_items(md_stream, format: :index, to: tmp_dir),
               &File.exists?/1
             )

      assert Enum.all?(
               Publish.write_items(md_stream, format: :markdown, to: tmp_dir),
               &File.exists?/1
             )

      assert Enum.all?(
               Publish.write_items(md_stream, format: :saml, to: tmp_dir),
               &File.exists?/1
             )

      assert Enum.all?(
               Publish.write_items(md_stream, format: :thiss, to: tmp_dir),
               &File.exists?/1
             )

      assert Enum.all?(
               Publish.write_items(md_stream, format: :udest, to: tmp_dir),
               &File.exists?/1
             )

      assert Enum.all?(
               Publish.write_items(md_stream, format: :udisco, to: tmp_dir),
               &File.exists?/1
             )
    end
  end
