defmodule SmeePublishSamlXmlTest do
  ...
  describe "eslength/2" do
    test "returns the size of content in the stream" do
      assert 41_458 = ThisModule.eslength(Metadata.stream_entities(@valid_metadata))
    end
  ...
  describe "aggregate_stream/2" do
    ...
    test "the third chunk is a XML record" do
      ...
      assert "<EntityDescriptor entityID=\"https://test.ukfederation.org.uk/entity\">" <> _ =
               record
    end
  ...
  describe "aggregate/2" do
    ...
    test "should include minimal EntityDescriptor tags" do
      ...
      assert String.contains?(
               xml,
               ~s|<EntityDescriptor entityID="https://test.ukfederation.org.uk/entity">|
             )

      assert String.contains?(
               xml,
               ~s|<EntityDescriptor entityID="https://indiid.net/idp/shibboleth">|
             )
    end
  ...
  describe "write_aggregate/2" do
    ...
    test "the file is valid", %{filename: filename} do
      file = File.read!(filename)
      assert {:ok, ^file} = Lint.validate(file)
    end
  end
  ...
  describe "write_items/2" do
    ...
    test "the files contain the right entities", %{filenames: filenames} do
      for filename <- filenames do
        ...
        assert String.contains?(file, "https://test.ukfederation.org.uk/entity") ||
                 String.contains?(file, "https://indiid.net/idp/shibboleth")
      end
    end
